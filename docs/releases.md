...
- Creating a stable release tag freezes every migration included in that tag. Later schema changes must use a new numbered migration.
...
If a release attempt is abandoned before publication, delete its tag before changing a migration introduced since the preceding release.
